...
## Stephen Fickas, June 2026 (updated July 30)

## Project Overview

This project develops automated detection of *Sesarma reticulatum* (purple marsh crab) burrow damage along salt-marsh channels using drone-collected high-resolution imagery. Crab burrowing is a key driver of New England marsh die-off and currently has to be mapped by hand from aerial photos or kayak surveys — neither approach scales to the regional monitoring we need.

The pipeline is built to be **marsh-agnostic**: nothing in the method is tied to a particular site. It takes drone orthoimagery, a DEM, and a set of labeled polygons, and derives everything else (bands, splits, thresholds) from the data itself. Our first imagery is expected from Wellfleet, MA, but flights from other marshes — Barnstable, for example — are also possible, and the pipeline is ready to ingest whatever marsh data we receive without reconfiguration. This is by design: the goal is a reusable regional monitoring method, not a one-marsh model.
...
**On confidence calibration.** A model can be accurate but *overconfident* — reporting 0.9 probability on predictions that are right only 80% of the time. Because our downstream decision rules (per-class thresholds, the abstain margin) all read the softmax probabilities directly, it matters whether those probabilities are trustworthy. We therefore measure calibration with the Expected Calibration Error (ECE) and, when warranted, fit a single temperature-scaling parameter on the validation split to correct it. The correction is applied conservatively: we report the model as-is when ECE is already low (under 3%), and only apply temperature scaling when it both exceeds that bar and demonstrably improves. This calibration analysis is a *reporting* diagnostic that characterizes the model's probability scale; the per-class thresholds are tuned empirically on that same scale, so the precision guarantees hold regardless. (On the current synthetic model, ECE is ~2% — already well-calibrated — so no correction is applied.)
...
### 9. Marsh Labeler — Multi-User Review Tool

The active-learning loop above needs a human to answer the questions the model raises — and doing that in raw QGIS is slow, error-prone, and hard to coordinate across people. So we built a purpose-designed web application, **Marsh Labeler**, that is the human end of the loop. It takes the review queue (the promoted, contested superpixels) and presents each one as a focused, answerable question rather than a blank labeling canvas.

For each review container the tool shows the exact question the model is stuck on — e.g. *"Model can't decide. Is this healthy_bank or crab_edge?"* — with the container outlined on the imagery, flanked by **confident reference tiles** for each candidate class (examples the model *is* sure about), so the labeler judges the ambiguous case against clear anchors rather than from memory. Supporting detail is on-screen: the per-superpixel breakdown of confident vs. unsure pixels, the abstain fraction, the container's class purity, and switchable image views (true-color, panchromatic, false-color/CIR) so the labeler can look at the same ground through different bands. Labeling is keyboard-driven for speed, with escape hatches for the cases the two-way question doesn't cover — assign a different class entirely, mark *needs split* where a container straddles a real boundary, flag *out of scope*, or skip.

**Multi-user, for defensible ground truth.** The tool is built for multiple labelers, each classifying the contested set **independently**. A review item is not treated as resolved on one person's say-so — it *settles* only when independent labels agree (or are adjudicated). This gives two things a single-labeler workflow cannot: a measure of **inter-annotator agreement** (how genuinely ambiguous each case is, straight from whether trained people concur), and a ground-truth set that reflects consensus rather than one expert's judgment. Work is organized into rounds (label → retrain → a fresh, smaller queue), and progress is tracked per task, so the team can see how much of the contested set has settled at any point. Tasks are organized by marsh, resolution, and round — consistent with the marsh-agnostic design, the tool already anticipates multiple sites.

The application is deployed, live, and **access-controlled** — labelers sign in, so the label set and the imagery are not publicly modifiable.

**Live at:** https://marsh-labeler-production.up.railway.app (sign-in required; credentials available on request)

*(The tool is currently populated with a synthetic demonstration task pending real flight data.)*

### 10. Web Portal — Companion Site

**Live at:** https://marsh-portal-production.up.railway.app/site/

The pipeline described above produces rasters, polygons, and metrics that live in notebooks and GIS files — legible to the modeling team, but not to the broader group of collaborators and agency partners who need to understand and trust what the method does. To close that gap, we built a companion web portal: a deployed, browsable site that presents the method, the imagery, and the results in a form a non-specialist can navigate.

The portal is a Django/GeoDjango web application, deployed and live, backed by a PostGIS spatial database and object storage for imagery and footage. It is a *modeling companion* to the parent MassMarsh crab-damage project — a place to show what the pipeline does and how well it works, not a replacement for the GIS deliverables.

Current contents:

- **Study Site** — a geographic map of the Wellfleet study area with the kayak-survey observation points that anchor our understanding of on-the-ground conditions, and the real drone-flight footprint overlaid.
- **Damage Map** — an interactive map that renders the model's output as per-class polygons over satellite and street basemaps, with per-class toggles and a legend. It distinguishes confident damage from the two uncertainty states the pipeline produces (under-review vs. no-decision), so a viewer can see not just where the model found damage but where it declined to commit. (The current map is populated with a synthetic demonstration pending real flight data.)
- **Results** — the evaluation page: per-class performance, the calibration analysis (reliability and ECE), and the per-class precision/recall/threshold tables that show, class by class, how confident the model has to be before a prediction is trusted. This is where the rigor of the method is made visible.
- **Methods / Pipeline** — a plain-language walkthrough of the two-tier approach and the active-learning labeling loop.
- **Related Work** — the literature the method builds on (the two-pass paradigm, crab-damage and burrow-mapping work, and the calibration methods), grouped and annotated.
- **Equipment** — the sensor and platform used for data capture.

The site is content-managed, so collaborators' text, imagery, and results can be updated without code changes, and new evaluation runs or damage maps can be loaded as real data arrives. Its purpose is threefold: to make the method legible to non-AI collaborators, to give agency partners something concrete to evaluate, and to serve as a living record of results as the project moves from synthetic to real data.
...
- Possible oblique-angle flight (under discussion). Much of the most diagnostic crab damage — dense burrowing — occurs on the near-vertical face of creek banks. A straight-down (nadir) camera views that face nearly edge-on and may under-resolve it, whereas an oblique view (like the kayak vantage, but from the air) sees it directly. If a follow-on oblique-camera flight materializes, it would let us test whether bank-face burrow damage is better captured obliquely — a question our current nadir flights are well-positioned to raise but not to answer.
...
| Confidence calibration (temperature / ECE reporting) | Complete |
| Marsh Labeler (multi-user review tool) | Complete, deployed and live |
| Web portal (companion site) | Complete, deployed and live |
...
**Calibration / ECE (Expected Calibration Error)** — Calibration is whether a model's stated probabilities match reality: among pixels it calls 80%-confident, are ~80% actually correct? ECE summarizes the mismatch in a single number. An overconfident model can be corrected with temperature scaling — dividing the logits by a single learned constant to soften the probabilities — without changing which class wins.
